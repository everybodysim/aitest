...
const nameTagLayer = document.createElement('div');
nameTagLayer.style.position = 'fixed';
nameTagLayer.style.inset = '0';
nameTagLayer.style.pointerEvents = 'none';
nameTagLayer.style.zIndex = '10';
...
const nameTags = agents.map((agent, i) => {
  ...
  el.textContent = `A${i + 1}`;
  el.style.position = 'absolute';
  el.style.padding = '1px 5px';
  el.style.border = '1px solid rgba(15,23,42,0.7)';
  ...
  return { agent, el, baseColor: rgb, index: i + 1 };
});
...
function updateNameTags() {
  for (const item of nameTags) {
    const head = item.agent.bodies.head.position;
    const p = new THREE.Vector3(head.x, head.y + 0.35, head.z).project(camera);
    const visible = p.z > -1 && p.z < 1;
    item.el.style.display = visible ? 'block' : 'none';
    const d = camera.position.distanceTo(new THREE.Vector3(head.x, head.y, head.z));
    const fade = THREE.MathUtils.clamp(1 - (d - 10) / 70, 0.25, 1);
    item.el.style.opacity = `${fade}`;
    item.el.textContent = `A${item.index} avg:${item.agent.getAverageReward().toFixed(2)}`;
