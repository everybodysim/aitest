...
export class HumanoidAgent {
  constructor(id, world, scene, color, spawn, terrainHeightFn) {
    ...
    this.partOffsets = Object.fromEntries(PART_DEFS.map((p) => [p.key, new CANNON.Vec3(...p.offset)]));
    this.prevActions = new Array(JOINTS.length).fill(0);
    this.lifetimeRewardSum = 0;
    this.lifetimeRewardCount = 0;
  ...
  #buildBodies() {
    ...
    for (const part of PART_DEFS) {
      ...
      body.position.set(this.start.x + part.offset[0], this.start.y + part.offset[1], this.start.z + part.offset[2]);
  ...
  #buildJoints() {
    for (const [name, a, b, worldPivot, axis, maxTorque] of JOINTS) {
      ...
      // Joint anchors are defined in model space and converted into each part's local frame.
      const pivotModel = new CANNON.Vec3(...worldPivot);
      const offsetA = this.partOffsets[a];
      const offsetB = this.partOffsets[b];
      const pivotA = pivotModel.vsub(offsetA);
      const pivotB = pivotModel.vsub(offsetB);
  ...
  applyActions(actions) {
    for (let i = 0; i < this.joints.length; i += 1) {
      ...
      const raw = THREE.MathUtils.clamp(actions[i] || 0, -1, 1);
      const maxDelta = 0.22;
      const limited = THREE.MathUtils.clamp(raw, this.prevActions[i] - maxDelta, this.prevActions[i] + maxDelta);
      this.prevActions[i] = limited;
      const torque = limited * j.maxTorque;
  ...
  getAverageReward() {
    if (this.lifetimeRewardCount === 0) return 0;
    return this.lifetimeRewardSum / this.lifetimeRewardCount;
  }
  ...
  getFinesse() {
    ...
    const upright = Math.max(0, Math.min(1, (upY + 0.1) / 1.1));
    const bodyList = Object.values(this.bodies);
    const avgAngVel = bodyList.reduce((acc, b) => acc + b.angularVelocity.length(), 0) / bodyList.length;
    const smoothness = Math.max(0, 1 - avgAngVel / 5.5);
    const stride = Math.max(0, Math.min(1, Math.abs(torso.velocity.x) / 2.4));
    return Math.max(0, Math.min(100, (upright * 0.45 + smoothness * 0.35 + stride * 0.2) * 100));
  }
  ...
  updateMetrics(dt) {
    ...
    const q = torso.quaternion;
    const upY = 1 - 2 * (q.x * q.x + q.z * q.z);
    const tooLow = torso.position.y < floorY + 0.65;
    const tipped = upY < 0.35;

    if (tooLow || tipped) {
      this.stats.fallenTime += dt;
    } else {
      this.stats.fallenTime = Math.max(0, this.stats.fallenTime - dt * 2);
    }

    if (torso.position.y < floorY - 1.6 || this.stats.fallenTime > 2.2) {
      this.stats.reward -= 5;
      this.reset();
      return true;
    }
